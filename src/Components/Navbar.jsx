...
const Navbar = ({ activeModule, setActiveModule }) => {
  const { user, logout, canAddMachine, canRecordDailyLog, canRecordMaintenance, canViewReports, canViewPerformance, getRoleDisplayName, getUserInitials } = useUser();
  ...
  return (
    <nav className="bg-white shadow-md fixed w-full top-0 z-50 border-b border-gray-200">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between h-16">
          ...
          <div className="flex items-center space-x-4">
            {/* Notifications */}
            <button className="relative p-2 text-gray-600 hover:text-blue-700 hover:bg-blue-50 rounded-full transition-colors">
            ...
            <div className="relative">
              ...
              >
                ...
                <div className={`w-8 h-8 bg-gradient-to-r from-blue-500 to-blue-600 rounded-full flex items-center justify-center shadow-sm ${user?.avatar ? 'hidden' : ''}`}>
              ...
              {isProfileMenuOpen && (
                <div className="absolute right-0 mt-2 w-64 bg-white rounded-md shadow-lg py-1 z-50 border border-gray-200">
                  ...
                  <div className="border-t border-dairy-100"></div>
                  <button
                    onClick={() => {
                      setIsProfileMenuOpen(false);
                      logout();
            ...
            <button
              ...
              className="lg:hidden p-2 rounded-md text-gray-600 hover:text-dairy-700 hover:bg-blue-50 transition-colors"
            >
              <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      {isMobileMenuOpen && (
        <div className="lg:hidden bg-white border-t border-dairy-200">
            ...
            {navItems.map((item) => (
              <button
                ...
                className={`w-full flex items-center space-x-3 px-3 py-2 rounded-md text-left font-medium transition-colors ${
                  activeModule === item.id
                    ? 'bg-dairy-100 text-dairy-700 border border-dairy-200 shadow-sm'
                    : 'text-gray-600 hover:text-dairy-700 hover:bg-blue-50'
                }`}
              >
                {item.icon}
                <span>{item.name}</span>
